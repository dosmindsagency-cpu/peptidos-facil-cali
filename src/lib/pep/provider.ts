import { PEP_MODEL, PEP_SYSTEM_PROMPT } from "@/lib/pep/system";
...
export async function requestPepProvider(history: PepChatMessage[], message: string, stream: boolean, signal: AbortSignal) {
...
export function providerError(status: number) {
  if (status === 401 || status === 403) return "Pep no está disponible temporalmente.";
  if (status === 429) return "Pep está recibiendo muchas preguntas. Inténtalo de nuevo en unos minutos.";
  if (status >= 500) return "Pep no pudo conectarse con su servicio de respuestas.";
